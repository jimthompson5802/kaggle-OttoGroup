...
CARET.TRAIN.PARMS <- list(method="svmRadial",
...
MODEL.SPECIFIC.PARMS <- NULL #list(verbose=FALSE)

MODEL.COMMENT <- "svm one vs all"
...
library(doSNOW)
cl <- makeCluster(2)
clusterExport(cl,c("logLossEval"))
...
Sys.time()
set.seed(825)
time.data <- system.time(all.mdls<-lapply(PRODUCT.CLASSES,trainForOneClass,
                                          train.data$predictors,
                                          train.data$response))
time.data
names(all.mdls) <- PRODUCT.CLASSES
comment(all.mdls) <- MODEL.COMMENT
...
ll <- lapply(PRODUCT.CLASSES,predictForOneClass,all.mdls,test.data$predictors)
...
ll <- plyr::llply(all.mdls,function(x){print(x$bestTune)})
...
if (last.idx == 1 || improved == "Yes") {
    ...
    file.name <- paste0("/model_",all.mdls[[1]]$method,"_one_vs_all_",modelPerf.df$date.time[last.idx],".RData")
    file.name <- gsub(" ","_",file.name)
    file.name <- gsub(":","_",file.name)
    
    save(all.mdls,file=paste0(WORK.DIR,file.name))
} else {
    cat("no improvement!!!\n")
    flush.console()
}
