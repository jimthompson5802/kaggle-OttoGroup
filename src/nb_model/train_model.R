...
WORK.DIR <- "./src/nb_model"  # modify to specify directory to contain model artififacts
...
CARET.TRAIN.PARMS <- list(method="nb")   # Replace MODEL.METHOD with appropriate caret model

# CARET.TUNE.GRID <-  NULL  # NULL provides model specific default tuning parameters

# user specified tuning parameters
CARET.TUNE.GRID <- expand.grid(usekernel=c(TRUE,FALSE),
                               fL=c(0,1e-8))
